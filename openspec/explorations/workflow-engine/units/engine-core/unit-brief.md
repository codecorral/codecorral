...
The workspace config declaratively sets the agent-deck profile (which governs session management, worktree conventions, and conductor identity) and the Claude Code profile (model selection, API keys) for all sessions created within that workspace. This means switching between projects with different API keys or model preferences is a workspace-level concern, not per-session.
