...
The engine SHALL read workspace configuration from `~/.codecorral/config.yaml`. The config file SHALL define workspaces as a map keyed by workspace name, where each workspace specifies a `path`, optional `workflows` list, optional `agentDeck` profile config, optional `claudeCode` profile config, optional `openspec` schema config, optional `board` URL string, and optional `conductor` config.

#### Scenario: Parse valid config file
- **WHEN** the engine reads a `config.yaml` with two workspace entries
- **THEN** each workspace is loaded with its path, workflows, agentDeck profile, claudeCode profile, openspec configuration, board URL, and conductor configuration
...
### Requirement: Conductor configuration per workspace
Each workspace MAY declare a `conductor` section with `name` (the conductor session title used with `agent-deck attach`). If omitted, the command center terminal pane opens a shell instead of attaching to a conductor.

#### Scenario: Workspace with conductor name
- **WHEN** a workspace defines `conductor.name: conductor-myproject`
- **THEN** the engine stores the conductor name as part of the workspace configuration

#### Scenario: Workspace without conductor config
- **WHEN** a workspace does not define a `conductor` section
- **THEN** the conductor name defaults to null and the command center terminal pane opens a shell
